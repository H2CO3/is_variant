//! Errors potentially happening while `#[derive]`ing `is_XXX` methods.
...
/// Convenience type alias for a result that holds an `is_variant::Error` value.
pub type Result<T> = result::Result<T, Error>;

/// An error that potentially happens while `#[derive]`ing `is_XXX` methods.
#[derive(Debug)]
pub struct Error {
    /// The error message.
    message: String,
